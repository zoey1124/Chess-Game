...
class BoardWidget extends Pad {
    ...
    static final Color
        SQUARE_COLOR = new Color(238, 197, 213),
        THRONE_COLOR = new Color(255, 137, 172),
        ADJACENT_THRONE_COLOR = new Color(214, 220, 211),
        KING_COLOR = new Color(178, 167, 255),
    ...
    private void drawGrid(Graphics2D g) {
        ...
        g.fillRect(cx(Board.THRONE),
                cy(Board.THRONE), SQUARE_SIDE, SQUARE_SIDE);
    ...
    private void drawPiece(Graphics2D g, Square s) {
        ...
        if (side != EMPTY) {
            g.setColor(side.side() == WHITE
                    ? (side == KING
                    ? KING_COLOR : WHITE_COLOR)
                    : BLACK_COLOR);
            g.fillOval(cx(s) + F, cy(s) + F, T, T);
        }
    }

    /** Handle a click on S. */
    private void click(Square s) {
        if (firstClick == null && _board.get(s) != EMPTY) {
            firstClick = s;
        }

        if (firstClick != null) {
            if (_board.isLegal(firstClick, s)) {
                _board.makeMove(firstClick, s);
                firstClick = null;
            }
            firstClick = s;
        }
    ...
    /** Represent the first clicked square (if there is any). */
